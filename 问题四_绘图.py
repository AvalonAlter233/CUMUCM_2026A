...
def main() -> None:
    times, fixed_radius_cm, moisture = read_result()
    history = problem4.read_radius_history(problem4.RADIUS_FILE)
    surface_radius_cm = np.asarray(
        [100.0 * problem4.radius_value(time * 3600.0, history) for time in times]
    )
    ...
    for selected in selected_hours:
        ...
        radii = np.concatenate([fixed_radius_cm, [surface_radius_cm[index]]])
        label = (
            f"{times[index]:.4f} h（首次离散达标）"
            if index == threshold_index
            else f"{times[index]:.0f} h"
        )
        axis.plot(radii, moisture[index], marker="o", ms=2.5, label=label)
    ...
    normalized_field = np.empty((len(times), len(xi_grid)))
    for index in range(len(times)):
        sample_xi = np.concatenate(
            [fixed_radius_cm / surface_radius_cm[index], [1.0]]
        )
        normalized_field[index] = np.interp(
            xi_grid, sample_xi, moisture[index]
        )
    figure, axis = plt.subplots(figsize=(7.2, 4.2))
    image = axis.imshow(
        normalized_field.T,
    ...
    if DIAGNOSTICS_FILE.exists():
        ...
        cases = [cases_by_name[name] for name in ordered_names]
        labels = [case["label"] for case in cases]
        values = [case["continuous_threshold_time_h"] for case in cases]
        figure, axis = plt.subplots(figsize=(7.2, 4.2))
        bars = axis.bar(labels, values, color=["#7f8c8d", "#1f77b4", "#d62728"])
